...
function findUpowerRoot() {
  ...
  const candidates = [
    ...
    path.resolve(__dirname, '../..'),
    path.resolve(process.cwd(), '.upower'),
    process.cwd()
  ];

  for (const candidate of candidates) {
    if (
      fs.existsSync(path.join(candidate, 'scaffold', 'bin')) &&
      fs.existsSync(path.join(candidate, 'scaffold', 'templates'))
    ) {
      return candidate;
    }
...
if (!fs.existsSync(sourceDir)) {
  ...
  if (!fs.existsSync(templateDir)) {
    templateDir = path.join(upowerRoot, 'templates', 'Object_Name');
  }
  if (!fs.existsSync(templateDir)) {
